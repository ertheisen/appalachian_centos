...
for i in range(len(input_files_R1)):
	...
	temp_str = 'bowtie2 --no-unal --no-mixed --no-discordant --no-dovetail --phred33 -q -I 10 -X 500 --threads 16' \
...
for i in range(len(datafiles)):
	...
	subset_big = temp_bed_stripped[(temp_bed_stripped.iloc[:,3]>=301) & (temp_bed_stripped.iloc[:,3]<=500)]
